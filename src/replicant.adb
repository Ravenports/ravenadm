...
package body Replicant is
   ...
   begin
      ...
      if DIR.Exists (location (slave_base, tmp) & toolchain_tag) then
         unhook_toolchain (id);
      end if;
   ...
   begin
      ...
      TIO.Create (File => forged,
                  Mode => TIO.Out_File,
                  Name => location (slave_base, tmp) & toolchain_tag);
      TIO.Close (forged);
   ...
   begin
      ...
      DIR.Delete_File (location (slave_base, tmp) & toolchain_tag);
